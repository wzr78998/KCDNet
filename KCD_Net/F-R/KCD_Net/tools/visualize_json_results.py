...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default='/home/skj/zw/VIS/F-R/SparseR-CNN-main/projects/KCDNet/output/1',help="output directory")
    parser.add_argument("--dataset", help="name of the dataset", default="M3FD_val")
    ...
    for dic in tqdm.tqdm(dicts):
        ...
        for k in range(len(dic['annotations'])):
            ...
            for i in range(predictions.pred_classes.shape[0]):
                box_p=np.array(predictions.pred_boxes[i].tensor.numpy(),dtype=np.int32).tolist()
                ious=iou(box,box_p)
                if ious>=0.5:
                    pre.append(predictions.pred_classes[i])
                num2 = len(pre)
                if num2==num1+1:
                    break
    ...
    np.save('./pre.npy',np.array(pre))
    np.save('./label.npy', np.array(label))



    cv2.imwrite(os.path.join(args.output, basename), vis_pred[:, :, ::-1])
    cv2.imwrite(os.path.join(str(args.output[:-3]+'ir'), basename), ir_pred[:, :, ::-1])
    cv2.imwrite(os.path.join(str(args.output[:-3] + 'gt'), basename), vis_gt[:, :, ::-1])
